...
#else*/
uint8_t pinMap[16]={12,13,14,15,16,17,18,19,2,3,4,5,6,9,10,11};
#endif
...
void setup()
{
  ...
  else
  {
    ...
    for (n=0;n<16;n++)
    {
      ...
      {
        #ifdef DEBUG
        Serial.print("Pin ");Serial.print(pinMap[n]); Serial.print(" salida "); Serial.print(n); Serial.println(" como SALIDA");
        #endif 
      ...
      {
        #ifdef DEBUG
        Serial.print("Pin ");Serial.print(pinMap[n]); Serial.print(" salida "); Serial.print(n); Serial.println(" como INPUT_PULLUP");
        #endif
  ...
  Serial.print("Modulo ");Serial.print(svtable.svt.addr_low);Serial.print("/");Serial.println(svtable.svt.addr_high);
}
...
{  
  ...
  for (n=0; n<16; n++)
  {
    if (!bitRead(svtable.svt.pincfg[n].cnfg,7))   //Setup as an Input
    {
      ...
      {
        #ifdef DEBUG
        Serial.print("ENTRADA ");Serial.print(n);
        Serial.print(" EN EL PIN "); Serial.print(pinMap[n]);
...
void notifySensor( uint16_t Address, uint8_t State )
{
  ...
  Serial.print(" - ");
  Serial.println( State ? "Activo" : "Inactivo" );
  #endif
}
...
void notifySwitchRequest( uint16_t Address, uint8_t Output, uint8_t Direction )
{
  ...
  Serial.print(Address, DEC);
  Serial.print(':');
  Serial.print(Direction ? "Cerrado" : "Lanzado");
  Serial.print(" - ");
  ...
  for (n=0; n<16; n++)
  {
    ...
    {
      #ifdef DEBUG
      Serial.print("Salida asignada al puerto ");
      Serial.println(n);
      #endif
...
boolean processPeerPacket()
{
  ...
  {
    #ifdef DEBUG
    Serial.println("OPC_PEER_XFER no para mi!");
    Serial.print("LnPacket->px.dst_l: ");Serial.print(LnPacket->px.dst_l);Serial.print(" Dirección low: ");Serial.println(svtable.svt.addr_low);
    Serial.print("LnPacket->px.d5: ");Serial.print(LnPacket->px.d5);Serial.print(" Dirección high: ");Serial.println(svtable.svt.addr_high);
    Serial.print("LnPacket->px.dst_h: ");Serial.print(LnPacket->px.dst_h);Serial.print(" Dirección high: ");Serial.println(svtable.svt.addr_high);
  ...
  {
    #ifdef DEBUG
    Serial.print("LEER ");Serial.print(LnPacket->px.d2);Serial.print(" ");Serial.print(LnPacket->px.d2+1);Serial.print(" ");Serial.println(LnPacket->px.d2+2);
    #endif
  ...
  if (LnPacket->px.d1==1)
  {
    ...
    #ifdef DEBUG
    Serial.println(">> OPC_PEER_XFER respuesta enviada");
    #endif
    return (true);
  }
...
void sendPeerPacket(uint8_t p0, uint8_t p1, uint8_t p2)
{
  ...
  #ifdef DEBUG
  Serial.println("OPC_PEER_XFER Paquete enviado!");
  #endif
}
